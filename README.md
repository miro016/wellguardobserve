...
- A pannable, zoomable evidence-linked topology with architecture, network-routing, and full-evidence lenses. It can expand into a full-viewport operations workspace from the toolbar or with `F`, and `Esc` restores the normal workspace. Hovering an asset highlights its complete directed upstream and downstream path without lighting sibling branches. The primary architecture projection is authorized host → observed server/address → one port per server → domain routed through that port → service; provider, URL, registration, and raw-evidence nodes remain available in the other lenses without crowding the ownership tree. A time rail replays immutable snapshots and overlays newly observed, changed, and not-observed assets. Branch focus/collapse, tag-filtered portfolio maps, saved local views, SVG export, and print-to-PDF support larger estates.
...
- A platform-wide runtime tool registry. Administrators can view all compiled capabilities, disable non-essential tools, and grant or revoke each tool for each scan profile. They can also create, disable, and remove reviewed declarative probes. The worker resolves this policy before every scan and records the effective tool set in the immutable profile snapshot.
- A pinned Vanguard evidence-collection integration for Active validation and stronger profiles. Its free-provider Wellguard profile disables paid lookups, AXFR, SMTP, independent port scanning, GoScans, and out-of-scope requests; collection and offline projection results are retained as ordinary tool output and imported into the asset graph.
...
- A live job console with a five-second worker heartbeat, current phase, full model conversation, separately labelled provider-emitted thinking text, tool inputs/results, delayed/stalled indicators, safe user cancellation, and explicit recovery of jobs interrupted by a single-instance worker restart; evidence already retained remains auditable after a stop.
- Content-validated path probing: bounded candidate bodies are read, fingerprinted, previewed with secret-shaped values redacted, and compared with the root representation. SPA/default-page `200` responses, root redirects, and wrong media types are separated from verified distinct content. A scan-level guard stops before executing a fourth identical tool-and-parameters request.
...
| `WELLGUARD_VANGUARD_PROFILE` | Reviewed Vanguard collection profile | `/app/integrations/vanguard/profile.yaml` |
| `WELLGUARD_VANGUARD_COLLECT` | Pinned Vanguard collector executable | `/usr/local/bin/vanguard-collect` |
| `WELLGUARD_VANGUARD_PROJECTIONS` | Pinned Vanguard offline projector | `/usr/local/bin/vanguard-projections` |
...
- Vanguard at the pinned internal revision recorded in `third_party/vanguard/WELLGUARD_REVISION`; Wellguard runs its reviewed free-provider profile and imports a bounded deterministic projection
