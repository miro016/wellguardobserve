...
export class TargetDetailComponent implements OnInit {
  ...
  private async load(): Promise<void> { const id = this.route.snapshot.paramMap.get('id') || ''; try { const targets = await this.db.targets(); const target = targets.find((x) => x.id === id) || null; const [findings, tls, actions, scans] = await Promise.all([this.db.findings(id), this.db.tls(id, target?.hostname), this.db.agentActions({ targetId: id }), this.db.scans(id)]); this.target.set(target); this.findings.set(findings); this.tls.set(tls); this.actions.set(actions); this.scans.set(scans); } catch (e) { this.error.set(e instanceof Error ? e.message : 'Could not load target.'); } }
