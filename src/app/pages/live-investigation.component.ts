  ...
  template: `
    <div class="app-layout"><wg-app-sidebar /><main class="app-main live-investigation-page">
      ...
      @if (request(); as job) {
        ...
        <section class="trace-layout live-trace"><article class="panel transcript-panel"><div class="panel-heading"><div><span class="section-index">LIVE MODEL CHANNEL</span><h2>Conversation + thinking</h2></div><span class="evidence-count"><i class="live-dot"></i>{{ messages().length }} events</span></div><div class="transcript" aria-live="polite">@for (message of messages(); track message.id) { <article class="message" [attr.data-role]="message.role"><header><span>{{ message.role }}</span>@if (message.toolName) { <b>{{ message.toolName }}</b> }<time>{{ message.occurredAt | date:'HH:mm:ss' }}</time></header>@if (message.reasoning) { <section class="model-reasoning"><div><span>MODEL THINKING</span><small>provider-emitted · may be incomplete</small></div><pre>{{ message.reasoning }}</pre></section> }<pre>{{ message.content }}</pre></article> } @empty { <div class="live-wait"><i></i><strong>Waiting for the agent</strong><span>The worker will publish its instructions, provider-emitted thinking, and first decision here.</span></div> }</div></article>
