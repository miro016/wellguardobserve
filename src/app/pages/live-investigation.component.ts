  ...
  template: `
    <div class="app-layout"><wg-app-sidebar /><main class="app-main live-investigation-page">
      ...
      @if (request(); as job) {
        ...
        @if (!isActive()) { <section class="investigation-handoff panel"><div><span class="section-index">INVESTIGATION RETAINED</span><h2>{{ request()?.status === 'completed' ? 'Continue with the evidence' : 'Review the partial trace' }}</h2><p>{{ scan()?.summary || scan()?.error || request()?.error || 'The full transcript and completed tool results remain available.' }}</p></div><div><a class="button secondary" [routerLink]="['/app/surface']" [queryParams]="{ target: job.target }">Open surface map</a><a class="button primary" routerLink="/app/traces" [queryParams]="{ target: job.target }">Audit full trace →</a></div></section> }
      }
    </main></div>
  `,
