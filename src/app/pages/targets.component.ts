...
@Component({ selector: 'wg-targets', imports: [AppSidebarComponent, RouterLink, DatePipe], template: `
  <div class="app-layout"><wg-app-sidebar /><main class="app-main"><header class="app-header"><div><span class="app-breadcrumb">OBSERVE / TARGETS</span><h1>Authorized targets</h1><p>Every investigation is anchored to a verified or administrator-approved root.</p></div><span class="scope-lock">⌾ Scope enforcement active</span></header>
  @if (error()) { <div class="error-banner"><strong>Targets unavailable</strong><span>{{ error() }}</span></div> }
  <section class="panel inventory-panel"><div class="panel-heading"><div><span class="section-index">INVENTORY</span><h2>{{ targets().length }} monitored root{{ targets().length === 1 ? '' : 's' }}</h2></div><span class="evidence-count">Registration disabled in private preview</span></div>
    ...
    @empty { <div class="empty-state">No authorized targets. An administrator must create and authorize one in PocketBase.</div> }</div>
  </section>
