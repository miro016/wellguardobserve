...
import { Finding, KnowledgeObservation, KnowledgePattern, Scan, Target } from '../models';
      ...
      <section class="knowledge-layout">
        ...
        <aside class="panel pattern-inspector">
          @if (active(); as pattern) {
            ...
            <h3>Evidence examples</h3><ul class="evidence-list">@for (finding of examples(pattern); track finding.id) { <li><strong>{{ finding.asset }}</strong><span>{{ finding.summary }}</span></li> }</ul>
...
export class KnowledgeComponent implements OnInit {
  ...
  ngOnInit(): void {
    void Promise.all([this.db.targets(), this.db.findings(), this.db.scans(), this.db.knowledgeObservations()])
      .then(([targets, findings, scans, observations]) => { this.targets.set(targets); this.findings.set(findings); this.scans.set(scans); this.observations.set(observations); })
