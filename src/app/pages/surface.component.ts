...
@Component({ selector: 'wg-surface', imports: [AppSidebarComponent, InfrastructureGraphComponent, FormsModule, RouterLink, DatePipe], template: `
  ...
  @if (target(); as item) {
    <section class="panel topology-panel expanded"><div class="panel-heading"><div><span class="section-index">INTERACTIVE ASSET MAP</span><h2>{{ item.hostname }}</h2></div><div class="panel-actions"><span class="evidence-count">Hover traces the complete path · drag to pan</span><a [routerLink]="['/app/targets', item.id]">Target detail →</a></div></div><wg-infrastructure-graph [target]="item" [findings]="findings()" [tls]="tls()" [actions]="actions()" [assets]="assets()" [relations]="relations()" /></section>
