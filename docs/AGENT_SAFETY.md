...
- The Unbounded common-path sweep reads every non-404 response within a fixed body cap, compares its representation with the root response, and retains a redacted preview and SHA-256. A `200` response, redirect to `/`, root/SPA-shell match, access-control response, or HTML returned for a data-file candidate is not classified as verified file presence. Generated file-shaped probes likewise require a body marker or JSON-key assertion and cannot use `HEAD` as proof.
- A scan-level repetition guard canonicalizes tool parameters. Up to three identical model requests are tolerated (duplicate network work is served from the per-run result cache); a fourth ends the agent turn before any request from that turn executes.
...
Generated capabilities do not weaken that boundary. The model may propose an `http-probe-v1` document containing a small number of same-origin requests and data-only assertions; it cannot submit or execute source code. Server-owned validation rejects alternate origins, traversal, arbitrary headers, cookies, authorization material, callbacks, files, processes, regular expressions, and shell or JavaScript. Each accepted definition receives an immutable SHA-256 checksum. Unbounded investigations may execute a valid proposal in the non-production experimentation lane, while every other profile requires administrator approval and a compatible minimum-profile assignment. The runtime revalidates scope, checksum, method, body size, step ceiling, and profile on every execution and writes an audit receipt. See [Generated capability registry](TOOLS.md).

Service banners, HTML, JSON, documentation, fingerprints, and repository text are untrusted data. Prompts explicitly prohibit following instructions embedded in evidence. Public fingerprint packs are pinned to exact revisions, size-bounded, normalized, and limited to supported fields. Product names require direct response evidence and cannot be inferred from hostnames or generic tool notes. Tool policy is enforced in code regardless of model output. The transcript stores reasoning text explicitly returned by the configured model provider; it may be incomplete and is explanatory telemetry, never target evidence or an authorization signal.
