...
if ! /usr/local/bin/pocketbase superuser upsert "$POCKETBASE_SUPERUSER_EMAIL" "$POCKETBASE_SUPERUSER_PASSWORD" \
...
if [[ -n "${WELLGUARD_ADMIN_EMAIL:-}" && -n "${WELLGUARD_ADMIN_PASSWORD:-}" ]]; then
  ...
  if ! POCKETBASE_URL=http://127.0.0.1:8090 bun run /app/scripts/seed.ts >/dev/null \
    2> >(tee -a "$error_file" >&2); then
